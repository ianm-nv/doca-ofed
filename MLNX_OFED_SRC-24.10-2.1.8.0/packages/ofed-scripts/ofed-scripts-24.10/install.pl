...
if ($DISTRO =~ /OPENEULER|EULEROS2.0SP(9|1[0-9])|RHEL[89]|FC3|BCLINUX8|BCLINUX2[12]|KYLIN10$|KYLIN10SP2|OL[89]|ANOLIS|CTYUNOS/) {
    $python_docutils = "python3-docutils";
} elsif ($DISTRO =~ /RHEL7|BCLINUX7|OL7/) {
    $python_docutils = "python-docutils";
}

if ($DISTRO =~ /OPENEULER22|CTYUNOS23/) {
...
my $perl_generators = "";
if ($DISTRO =~ /RHEL[89] | OL[89] | BCLINUX8 | ^FC | ANOLIS | CTYUNOS | KYLIN | TENCENT | ALINUX/x) {
...
} elsif ($DISTRO =~ /RHEL[89] | OL[89] | FC3[0-9] | EULEROS2.0SP1[01] | ANOLIS | OPENEULER | ALINUX | MARINER/x) {
    $python_six = "python3-six";
}

my $python_sphinx = "python-sphinx";
if ($DISTRO =~ /RHEL[89] | OL[89] | FC3[0-9] | EULEROS2.0SP1[01] | ANOLIS | OPENEULER | ALINUX/x) {
...
} elsif ($DISTRO =~ /RHEL9|ANOLIS|ALINUX/) {
    $python_twisted = "";
}

my $python_zope_interface = "python-zope-interface";
if ($DISTRO =~ /MARINER | RHEL8 | OL[89] | FC3[0-9] | EULEROS2.0SP1[01] | OPENEULER/x) {
...
if ($DISTRO =~ /RHEL8 | OL8 | EULEROS2.0SP8 | EULEROS2.0SP12 | BCLINUX8 | FC29 | FC3. | ANOLIS | ALINUX | TENCENT/x) {
...
if (($with_bluefield or $with_ovs_dpdk or $with_openvswitch) and
        $DISTRO =~ /RHEL[78] | FC3[0-9] | OPENEULER20.03SP1 | ANOLIS | ALINUX/x) {
...
if ($kmp and ($DISTRO =~ m/XenServer|FC|WINDRIVER6|POWERKVM|BLUENIX1|KYLIN|BCLINUX2[12]|UOS|OPENEULER20.03SP[13]|OPENEULER22.03|PHOTON|MARINER|AZURE|CTYUNOS|TENCENT/ or $kernel =~ /xs|fbk|fc|debug/)) {
...
my @kernel_packages = ($kernel_rpm, "mlnx-ofa_kernel-devel", "mlnx-ofa_kernel-source", $knem_rpm, $knem_mods, $xpmem_mods, $kernel_mft_rpm, 'iser', 'srp', 'isert', 'mlnx-nfsrdma', 'mlnx-nvme', 'mlnx-rdma-rxe', 'fwctl');
...
my @hpc_user_packages = (@basic_user_packages, "ucx", "ucx-devel", "ucx-static", ,"ibacm", "librdmacm",
                        ...
                        "infiniband-diags", "ibutils2", "cc_mgr", "dump_pr", "ar_mgr", "mstflint", "perftest", "multiperf", @mpi_packages, "hcoll", "sharp");
...
my %packages_info = (
        ...
        'mpitests_openmpi' =>
            { name => "mpitests_openmpi", parent => "mpitests",
            selected => 0, installed => 0, rpm_exist => 0,
            # May be enabled in set_cfg:
        ...
        'openvswitch' =>
            { name => "openvswitch", parent=> "openvswitch",
             ...
             available => 1, mode => "user",
             dist_req_inst => [],
             dist_req_build => ["desktop-file-utils", "groff", "graphviz", "openssl-devel", "selinux-policy-devel", "python3-sphinx", "$python_twisted", "$python_zope_interface", "libcap-ng-devel", "$python_six", "unbound", "unbound-devel", "libpcap-devel", "$python3_devel", "$python_rpm_macros"],
...
if ($DISTRO =~ /MARINER|AZURE/) {
    $packages_info{'mlnx-nvme'}{'available'} = 0;
}

if ($DISTRO =~ /SLES | openSUSE | KYLIN | BCLINUX2[12] | CTYUNOS | OL8.[78] | UOS20 | RHEL7.6ALT/x) {
    for my $p ("xpmem", "$xpmem_mods", "libxpmem", "libxpmem-devel", "ucx-xpmem") {
...
sub set_cfg
{
    ...
    if ($name =~ /openmpi/ and $DISTRO !~ /MARINER|AZURE|PHOTON|FC35|RHEL10/) {
        $packages_info{'mpitests_openmpi'}{'available'} = 1;
        $packages_info{'mpitests_openmpi'}{'base_mpi_version'} = $version;
    }
}
...
sub add_enabled_pkgs_by_user
{
    ...
    for my $key ( keys %force_enable_packages ) {
        next if (($key =~ /mlx-steering-dump/) and
                ($DISTRO =~ /RHEL7|SLES12|XenServer|OL7|BCLINUX2[12]|KYLIN|OPENEULER20.03|CTYUNOS|UOS|ALIOS/));
...
sub set_availability
{
    ...
    if ($DISTRO !~ /RHEL[7-9] | FC3[0-9] | OPENEULER20.03SP1 | ANOLIS | ALINUX/x) {
        $packages_info{'openvswitch'}{'available'} = 0;
    ...
    if ($DISTRO =~ /RHEL|OL|FC|POWERKVM|EULER|BCLINUX|ALIOS|ALINUX/) {
        $have_numa_libs = 1;
    ...
    if ($gcc{'gcc'}) {
        ...
        if ($DISTRO !~ /MARINER|AZURE|PHOTON|FC35|RHEL10/) {
    ...
    if (not (
        ...
        (($DISTRO =~ /RHEL8.6|OL8|CTYUNOS23/) and ($arch =~ /x86_64|aarch64/)) or
        (($DISTRO =~ /RHEL8.[1]$/x) and ($arch =~ /x86_64/))
        )
    ) {
    ...
    if (($DISTRO !~ /RHEL8.[1346]$|OL8|RHEL9.3/) or ($arch !~ /x86_64|aarch64/)) {
    ...
    if (not ($DISTRO =~ /RHEL7.[7-9]|RHEL8.[13]$|RHEL9.[1-2]|OL8.[4678]/x and $arch eq 'x86_64') and
        not ($DISTRO =~ /RHEL8.[46] | CTYUNOS23/x and $arch =~ /x86_64|aarch64/x )) {
    ...
    if ($DISTRO =~ /EULER|MARINER2|AZURE/) {
...
sub set_existing_rpms
{
    ...
    for my $binrpm (<$RPMS/*.rpm>) {
        ...
        if ($rpm_name =~ /knem-modules|kernel-mft|iser|srp$|mlnx_en|mlnx-nfsrdma|mlnx-nvme|mlnx-rdma-rxe/ and
            $rpm_name !~ /kmp|kmod/) {
            if ($kmp) {
        ...
        # W/A for kmp packages that has only kmod and kmp rpms
            ...
            if (($rpm_arch eq $arch or $rpm_arch eq "noarch") and
                 ($rpm_name !~ /iser|srp$|mlnx-nfsrdma|mlnx-nvme|mlnx-rdma-rxe/)) {
                $main_packages{$rpm_name}{$ver}{'rpmpath'} = $binrpm;
                $packages_info{$rpm_name}{$ver}{'rpm_exist'} = 1;
...
sub check_linux_dependencies
{
    ...
    for my $package ( @selected_packages ) {
        ...
        for my $ver (keys %{$main_packages{$pname}}) {
                ...
                @dist_req_inst_packages = () if ($build_only);
...
sub build_kernel_rpm
{
    ...
    elsif ($name =~ /iser|srp|mlnx-nfsrdma|mlnx-nvme|mlnx-rdma-rxe|fwctl/) {
    ...
    for my $myrpm ( <$TMPRPMS/*.rpm> ) {
        ...
        if($kmp and $myrpm_name =~ /kmp|kmod/ and
           $myrpm_name =~ /kernel-mft-mlnx|iser|srp|mlnx-en|mlnx-nfsrdma|mlnx-nvme|mlnx-rdma-rxe|fwctl/) {
            $myrpm_name =~ s/kmod-//g;
            $myrpm_name =~ s/-kmp.*//g;
        }
...
sub build_rpm
{
    ...
    if (not $packages_info{$name}{$ver}{'rpm_exist'}) {
        ...
        if ( ($parent =~ /openmpi/) and ($DISTRO =~ /BCLINUX8|BCLINUX2[12]|KYLIN|RHEL9|OL9|UOS|ANOLIS|CTYUNOS|OPENEULER20.03SP1|TENCENT|ALINUX/x) ) {
            $ldflags    .= " -fPIC";
...
sub install_kernel_rpm
{
    ...
    } elsif ($DISTRO =~ /CTYUNOS22/) {
        $release = "$release.ctl2";
    } elsif ($DISTRO =~ /CTYUNOS23/) {
        $release = "$release.ctl3";
    } elsif ($DISTRO =~ /UOS20.1060a/) {
        $release = "$release.uelc20";
    } elsif ($DISTRO =~ /UOS20.1060$/) {
        $release = "$release.uel20";
    }
