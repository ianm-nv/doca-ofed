...
sub mark_for_uninstall
{
	...
	if (not $selected_for_uninstall{$package}) {
		if (is_installed_deb $package) {
			...
			if ( not exists $remove_debs_hs{$_} and `ofed_info 2>/dev/null | grep -i $package 2>/dev/null` eq "" and $package !~ /ofed-scripts|mlnx-ofed-|neohost|infiniband-diags|doca-/) {
...
sub main
{
	...
	foreach (@remove_debs){
		...
		foreach (get_all_matching_installed_debs($_)) {
			if (not $selected_for_uninstall{$_} and $components_filter->($_)) {
				...
				if ( not exists $remove_debs_hs{$_} and `ofed_info 2>/dev/null | grep -i $_ 2>/dev/null` eq "" and $_ !~ /ofed-scripts|mlnx-ofed-|neohost|infiniband-diags|doca-/) {
	...
	if (not $dry_run) {
		...
		print_and_log "Running: apt-get install -y -f";
		system("apt-get install -y -f  >> $log 2>&1");
