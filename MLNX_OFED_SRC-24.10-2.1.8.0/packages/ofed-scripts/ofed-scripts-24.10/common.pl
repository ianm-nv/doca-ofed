...
sub parse_rpm_dist($$) {
	...
	if ($dist_rpm =~ /openSUSE-release-12.1/) {
		...
		if (
			...
			(($major eq "9") and ($minor <= 5)) ||
			(($major eq "10") and ($minor <= 0))
		) {
			$DISTRO = "RHEL$2.$3";
		...
		if (
			...
			(($major eq "8") and ($minor <= 8)) ||
			(($major eq "9") and ($minor <= 2))
		) {
			$DISTRO = "OL$1.$2";
			$rpm_distro = "ol$1u$2";
		}
	...
	} elsif ($dist_rpm =~ /euleros-release-2.0SP([23589]|1[0-2])/) {
		my $sp = $1;
		...
		$DISTRO = "ALIOS7.2";
		$rpm_distro = "alios7u2";
	} elsif ($dist_rpm =~ /kylin-release-10-24.6.(p85|p92)/) {
		$DISTRO = "KYLIN10SP3";
		$rpm_distro = "kylin10sp3";
		...
		$DISTRO = "KYLIN10";
		$rpm_distro = "kylin10";
	} elsif ($dist_rpm =~ /UnionTech-release-(10[26]0)/) {
		$DISTRO = "UOS20.$1";
		$rpm_distro = "uos20.$1";
	} elsif ($dist_rpm =~ /UnionTech-release-20-1060a/) {
		$DISTRO = "UOS20.1060a";
		$rpm_distro = "uos20.1060a";
