...
if ($with_ovs_dpdk and ($arch !~ /x86_64|aarch64/ or $distro !~ /debian1[0-2] | ubuntu2[0234].04/x)) {
	print YELLOW "\nWARNING: The '--ovs-dpdk' option is supported only on Ubuntu 20.04 and Debian10 and above, x86_64 and aarch64. Disabling...", RESET "\n";
	$with_ovs_dpdk = 0;
}

if ($with_gds and $distro !~ /ubuntu(18\.04|2[024]\.04)/) {
	$with_gds = 0;
}
...
my %packages_info = (
			...
			'rshim' =>
				{
				dist_req_build => ["autoconf", "autotools-dev", "pkg-config", "devscripts", "fakeroot", "libpci-dev", "libusb-1.0-0-dev", "libfuse-dev"],
...
$packages_info{"openvswitch-switch"}{'dist_req_inst'} = ["python3", "netbase", "procps", "uuid-runtime", "$libpcap", "libjansson4", "$libipsec_mb", "ifupdown"];
...
if ($distro =~ /debian|ubuntu24\.10/x) {
    for my $p ("xpmem", "$xpmem", "ucx-xpmem") {
...
sub is_module_in_deb
{
	...
	if ($deb) {
		if ($module =~ /srp|iser|sdp|rds|nfsrdma|mlnx-nvme|mlnx-rdma-rxe|fwctl/) {
			return 1;
		}
...
sub install_selected
{
	...
	foreach my $name (@selected_packages) {
		...
		} elsif (not $with_dkms and $parent =~ /iser|srp$|knem|kernel-mft|mlnx-en|mlnx-nfsrdma|mlnx-nvme|mlnx-rdma-rxe|fwctl|xpmem$/) {
			@debs = glob ("$DEBS/${deb_name}[-_]${version}-*.kver.${kernel}_*.deb");
			if (not @debs) {
				@debs = glob ("$DEBS/${deb_name}[-_]${version}-${kernel}_*.deb");
			}
		...
		if (not @debs) {
			...
			if (not $with_dkms and $parent =~ /mlnx-ofed-kernel|iser|srp$|knem|kernel-mft|mlnx-en|mlnx-nfsrdma|mlnx-nvme|mlnx-rdma-rxe|fwctl|xpmem$/) {
				$ENV{"WITH_DKMS"} = "0";
...
sub set_availability
{
	...
	if (($distro !~ /ubuntu2[024].04/x) or ($arch !~ /x86_64|aarch64/)) {
        ...
        if ($arch =~ /ppc/) {
                for my $package (qw/
                        xpmem-dkms xpmem-modules xpmem libxpmem0 libxpmem-dev
	...
	if ((($arch eq 'x86_64') and ($kernel !~ /
		...
		| ^5\.18\b
		| ^5\.19\b
		| ^6\.[0-8]\b	# Latest mainline
		/x) ) or
		(($arch eq 'aarch64') and ($distro !~ /ubuntu2[24]\.04/)) or
		($arch !~ /aarch64 | x86_64/x) or
		($distro =~ /debian12/)
	) {
		$kernel_modules_info{'nfsrdma'}{'available'} = 0;
		$packages_info{"$mlnx_nfsrdma"}{'available'} = 0;
	...
	if (not ($distro =~ /ubuntu18.04|ubuntu2[024].04/x and $arch =~ /x86_64|aarch64/x)) {
