...
int parse(struct dc_ctx *ctx, int argc, char *argv[])
{
	...
	int mtu, size_factor = 1, size_len;
        ...
        while (1) {
                ...
                c = getopt_long(argc, argv, "p:d:i:s:k:m:I:P:D:t:f:q:o:M", long_options, NULL);
                if (c == -1)
                        break;
...
int init_ctx(struct dc_ctx *ctx, int argc, char *argv[])
{
	ctx->port		= 18515;
	ctx->ib_port		= 1;
	ctx->dct_key		= 0x1234;
...
int init_test_data(struct dc_ctx *ctx, test_data *data)
{
	data->dct_num	= ctx->dct->dct_num;
...
static int create_mr(struct dc_ctx *ctx)
{
	...
	else
	{
		...
		reg_mr_exp_in.exp_access = IBV_EXP_ACCESS_REMOTE_WRITE | IBV_EXP_ACCESS_LOCAL_WRITE | IBV_EXP_ACCESS_REMOTE_READ | IBV_EXP_ACCESS_ALLOCATE_MR;
...
void print_bw_report(struct dc_ctx *ctx)
{
