...
void send_data_to_server(int sock, struct test_ctx *ctx)
{
	char msg[MSG_FORMAT_SIZE];
	sprintf(msg, MSG_FORMAT, ctx->qp[0]->qp_num, 0, ctx->portinfo.lid, 0, ctx->mr->rkey, ctx->mr->addr, 0, ctx->gid_index, ctx->mtu);
...
int modify_to_rts(struct test_ctx *ctx, test_data *data)
{
       struct ibv_qp_attr attr = {
                ...
                .path_mtu               = (enum ibv_mtu)data->mtu,
                .dest_qp_num            = data->qp_num,
                .rq_psn                 = data->psn,
...
int run_traffic(struct test_ctx *ctx, test_data *data)
{
	...
	sg_list.addr = (uintptr_t)ctx->addr;
	sg_list.length = ctx->size;
	sg_list.lkey = ctx->mr->lkey;
	wr.sg_list = &sg_list;
	wr.wr.rdma.remote_addr = data->vaddr;
	wr.wr.rdma.rkey = data->rkey;
	wr.wr_id = 0;
	...
	while (global_ctx->state != END_STATE)
	{
		for (i=1 ; i <= ctx->tx_depth; i++)
		{
			...
			err = ibv_exp_post_send(ctx->qp[0], &wr, &bad_wr);
			if (err) {
			...
			if (ctx->size <= (ctx->page_size / 2)) {
				increase_loc_addr(wr.sg_list,ctx->size,	totscnt,(uintptr_t)ctx->addr,0,ctx->cache_line_size,ctx->page_size);
				increase_exp_rem_addr(&wr,ctx->size,totscnt,data->vaddr,ctx->verb,ctx->cache_line_size,ctx->page_size);
			}
		}
		num = ctx->tx_depth;
		do {
...
int main(int argc, char *argv[])
{
	...
	if (pid == 0)
	{
		...
		send_data_to_server(sockfd, &ctx);
		...
		//sync, to let the server now I am done
		send_data_to_server(sockfd, &ctx);
