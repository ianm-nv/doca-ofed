...
int init_ctx(struct test_ctx *ctx, int argc, char *argv[])
{
	ctx->port		= 18515;
	ctx->ib_port		= 1;
	ctx->size		= 65536;
...
static int create_mr(struct test_ctx *ctx)
{
	...
	else
	{
		...
		reg_mr_exp_in.exp_access = IBV_EXP_ACCESS_REMOTE_WRITE | IBV_EXP_ACCESS_LOCAL_WRITE | IBV_EXP_ACCESS_REMOTE_READ;
