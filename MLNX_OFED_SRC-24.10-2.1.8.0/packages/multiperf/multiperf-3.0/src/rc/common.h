...
#define MSG_FORMAT "%06x:%06x:%04x:%06x:%08x:%16Lx:%08x:%06x:%04x"
#define MSG_FORMAT_SIZE 73
...
typedef enum {FETCH_AND_ADD, CMP_AND_SWAP} AtomicType;
...
void usage(const char *argv0)
{
	...
	printf("  -v  --verb		 Set Verb to use: WRITE / READ / ATOMIC\n");
	...
	printf("  -L  --mr_per_qp	 Create MR for each QP\n");
	printf("  -A  --atomicType	 Set Atomic type to use: CMP_AND_SWAP / FETCH_AND_ADD\n");
	printf("\n");
}
	...
	char *verb, *atomicType;
        ...
        while (1) {
                ...
                static struct option long_options[] = {
                        ...
                        { .name = "random",		.has_arg = 1, .val = 'R' },
                        { .name = "mr_per_qp",		.has_arg = 0, .val = 'L' },
			...
			{ .name = "atomic_type", 	.has_arg = 1, .val = 'A' },
                ...
                c = getopt_long(argc, argv, "p:d:i:s:x:m:I:P:D:t:f:q:o:v:Q:u:S:A:R:ML", long_options, NULL);
                if (c == -1)
                        break;
			...
			GET_STRING(ctx->ib_devname,strdupa(optarg));
                ...
                case 'v':
			GET_STRING(verb,strdupa(optarg));
		...
		case 'A':
			GET_STRING(atomicType, strdupa(optarg));
		...
		case 'R':
                        ctx->do_random_addr = strtol(optarg, NULL, 0);
			break;
...
int init_ctx(struct test_ctx *ctx, int argc, char *argv[], int to_parse)
{
	...
	if(to_parse)
	{
		...
		ALLOCATE(ctx->mr,struct ibv_mr*,ctx->num_of_qps);
	}
...
{
	printf("qp=%p, qp_num=%d, lid=%x, psn=%d, rkey=%x, srqn=%d, vaddr=%p, gid_index=%d, mtu=%d\n",(void *)data->qp, data->qp_num, data->lid, data->psn, data->rkey, data->srqn, (void *)data->vaddr, data->gid_index, data->mtu);
}
...
int create_single_mr(struct test_ctx *ctx, int i)
{
	...
	ctx->mr[i] = ibv_reg_mr(ctx->pd, ctx->addr, ctx->length,
...
int create_mr(struct test_ctx *ctx)
{
	...
	ctx->my_addr[0] = (uintptr_t)ctx->mr[0]->addr;
	...
	for (i = 1; i < ctx->num_of_qps; i++)
	{
		if (ctx->mr_per_qp)
		{
			...
			ctx->my_addr[i] = (uintptr_t)ctx->mr[i]->addr;
		}
		else
		{
			...
			ctx->mr[i] = ctx->mr[0];
			ctx->my_addr[i] = (uintptr_t)ctx->mr[0]->addr + i*BUFF_SIZE(ctx->size, ctx->page_size);
		}
	}
...
struct ibv_qp* ctx_qp_create_regular(struct test_ctx *ctx)
{
...
{
	return ctx_qp_create_regular(ctx);
}
...
int create_qps(struct test_ctx *ctx)
{
	...
	for (i = 0; i < ctx->num_of_qps; i++)
	{
		...
		struct ibv_qp_attr attr_init = {
			...
			.qp_access_flags = IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ | IBV_ACCESS_REMOTE_ATOMIC,
		};
