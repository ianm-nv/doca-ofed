...
dnl OVS_DISABLE_INTROSPECT
AC_DEFUN([OVS_DISABLE_INTROSPECT], [
  AC_REQUIRE([OVS_CHECK_DL])
  AC_ARG_ENABLE(introspect,
    [AS_HELP_STRING([--disable-introspect], [Disable built-in memory introspection])],
    [], [enable_introspect=yes])
  AM_CONDITIONAL([HAVE_INTROSPECT], [test "X$enable_introspect" != Xno])
  if test "X$enable_introspect" != Xno; then
    AC_DEFINE([HAVE_INTROSPECT], [1], [Define to 1 if introspect is enabled.])
  fi
  AC_SUBST([HAVE_INTROSPECT])
])

dnl OVS_WITH_SANITIZER
AC_DEFUN([OVS_WITH_SANITIZER], [
  AC_REQUIRE([OVS_DISABLE_INTROSPECT])
  AC_ARG_WITH(sanitizer,
    [AS_HELP_STRING([--with-sanitizer=address,undefined], [Enable runtime sanitizers])],
    [
        sanitize=yes
        case "$withval" in
        n | no) sanitize=no ;;
        "" | y | ye | yes) sanitizer="address,undefined" ;;
        *) sanitizer="$withval" ;;
        esac
    ])
  AM_CONDITIONAL([HAVE_SANITIZER], [test "X$sanitize" = Xyes])
  if test "X$sanitize" = Xyes; then
    if test "X$enable_introspect" != Xno; then
       AC_MSG_ERROR(m4_normalize([
          Runtime sanitizers are not compatible with embedded memory introspection.
          Use '--disable-introspect' to disable it.]))
    fi

    # Using OVS_CHECK_CC_OPTION is not possible due to invalid substitution
    # with some sanitizer config.
    SANITIZER_CFLAGS="-fsanitize=$sanitizer"
    AC_MSG_CHECKING([whether $CC accepts $SANITIZER_CFLAGS...])
    save_CFLAGS="$CFLAGS"
    ...
    AC_COMPILE_IFELSE(
        ...
        [
            AC_MSG_RESULT([no])
            AC_MSG_ERROR([invalid sanitizer configuration])
        ]
    )
    CFLAGS="$save_CFLAGS"
    # Improves sanitizer outputs if available.
    OVS_ENABLE_OPTION([-fno-omit-frame-pointer])
...
AC_DEFUN([OVS_CHECK_DPDK], [
  ...
  if test "$have_dpdk" != true || test "$with_dpdk" = no; then
    ...
    if test -n "$DPDK_PKGCONFIG"; then
        export PKG_CONFIG_PATH="${PKG_CONFIG_PATH}:${DPDK_PKGCONFIG}"
    fi
    echo "checking for DPDK in PKG_CONFIG_PATH='${PKG_CONFIG_PATH}'"
    case "$with_dpdk" in
    ...
    # Statically linked libraries might have been built with sanitizers enabled.
    # In such case, use the generated sanitizer cflags.
    CFLAGS="$CFLAGS $SANITIZER_CFLAGS $DPDK_INCLUDE"
