...
def traffic_poll_at_once(test, msg_size, iterations=10, opcode=e.IBV_WR_SEND):
    ...
    for i in range(iterations):
        ...
        test.client.mr.write(str(i) * msg_size, msg_size, i * msg_size)
    ...
    for i, j in zip(recv_id_order, send_id_order):
        exp_buff = bytearray((str(j) * msg_size), 'utf-8')
...
def odp_v2_supported(ctx):
    ...
    """
    from tests.mlx5_prm_structs import QueryHcaCapIn, QueryOdpCapOut, DevxOps
